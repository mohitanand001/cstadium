...
def validate_form(request):
    ...
    try:
        prob_obj1 = Problem.objects.get(
            problem_id=request.POST.get('problem_id'))
    except:
        prob_obj1 = None

    try:
        prob_obj2 = Problem.objects.get(
...
def prob_upload(request):
    ...
    if request.method == "POST":
        contest_obj = Contest.objects.get(
            name_of_contest=request.POST.get('contest_dropdown'))
        ...
        if validate_form(request) == True:
            ...
            problem_obj = Problem(problem_id=request.POST.get('problem_id'),
                                  name_of_problem=request.POST.get(
                                      'problem_title'),
            ...
            i = 0
            path_to_save = os.path.dirname(
                os.path.dirname(os.path.abspath(__file__)))
            path_to_save = os.path.join(
                path_to_save, 'media/problems/problem_' + str(p_id))
            print path_to_save
            ...
            for item in unzipped_file.namelist():
                if i <= 4:
                    file_ = file(path_to_save + '/input/input0' + str(i) +
                                 '.txt', 'w').write(unzipped_file.read(item))
                else:
                    file_ = file(path_to_save + '/output/output0' +
                                 str(i - 5) + '.txt', 'w').write(unzipped_file.read(item))
